...
namespace ctranslate2 {
  ...
  static void set_thread_affinity(std::thread& thread, int index) {
    ...
    // Android bionic does not expose pthread_setaffinity_np. The app leaves
    // cpu_core_offset at its default -1, so affinity is intentionally unused.
    (void)thread;
    (void)index;
#elif !defined(__linux__) || defined(_OPENMP)
    (void)thread;
    (void)index;
